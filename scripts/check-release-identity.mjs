...
function checkRavenBoundary(relativePath, source) {
  ...
  if (!/Raven/i.test(source) || !hasEnglishManual || !englishExtractsIntoParent || !englishExplainsInstalledRoot
...
function checkIdentityTemplate(relativePath, source, isDevelopment) {
  ...
  const identity = isDevelopment ? 'development' : 'stable';
  const hasIdentity = isDevelopment ? /development/i.test(source) : /stable/i.test(source);
  ...
  const staleIdentity = isDevelopment ? /\bstable\b/i : /\bdevelopment\b/i;
  if (versionLines.some(line => staleIdentity.test(line))) {
    const staleLabel = isDevelopment ? 'stable' : 'development';
