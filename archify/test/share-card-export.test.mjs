...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /var headerOffset = omitText[\s\S]*?SHARE_CARD_HEADER;/);
  ...
  assert.match(html, /function renderShareCard\(options\)[\s\S]*?serializeSvg\(sourceScale, \{[\s\S]*?routeSnapshot: routeSnapshot,[\s\S]*?reachSnapshot: reachSnapshot[\s\S]*?\}\)/);
